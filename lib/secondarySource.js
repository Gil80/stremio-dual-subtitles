 ...
 *
 * Queried ONLY as a fallback, never on the default/working path. Two
 * callers, both in addon.js:
 *   - `fetchAllSubtitles` (legacy non-Hebrew path) — implicit fallback,
 *     concatenates results into the primary list, so entries lose their
 *     source identity there. Fine, because that path never attributes
 *     entries per source.
 *   - the `mirror` row of `HEB_SOURCES` (Hebrew multi-source path) —
 *     explicit fallback; entries keep `source: 'mirror'` and the `label`
 *     below all the way through to the picker's SubtitlesName.
 */
