 ...
 *
 * The returned entry names the Hebrew side `hebSub` (not `mainSub`) on
 * purpose: which merge slot it ends up in depends on whether the user
 * configured Hebrew as mainLang or transLang — that decision belongs to
 * the caller (`assignHebrewSlots` in addon.js), not here.
 *
 ...
 * @returns {Array<{id: string, hebSub: object, fixedSub: object, source: string, label: string}>}
 */
function buildHebrewEntries(sourceGroups, fixedCandidate) {
